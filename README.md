...
CashVideo searches Jellyfin by the TMDB ID supplied by its catalogue. For shows it then resolves the selected season and episode. Native playback reports the current timestamp to CashVideo every five seconds, as well as on pause, completion, and player exit. The player back control, Escape key, backdrop exit, and browser Back all save the latest native timestamp before returning to the catalogue. Resume seeks to that timestamp the next time the same profile opens the title. Continue Watching cards show a red progress bar when a saved duration is available.
