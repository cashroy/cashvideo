...
function AdminPage({ currentUser }) {
  ...
  const [message, setMessage] = useState('');
  const [newUser, setNewUser] = useState({ username: '', displayName: '', pin: '', role: 'user' });
  ...
  async function addUser(event) { event.preventDefault(); try { await api('/api/admin/users', { method: 'POST', body: newUser }); setNewUser({ username: '', displayName: '', pin: '', role: 'user' }); setMessage('User created.'); load(); } catch (e) { setMessage(e.message); } }
